...
// ─────────────────────────────────────────────
// COMPONENTE FORA DO PAI — corrige perda de foco
// ─────────────────────────────────────────────
...
interface HabitFormFieldsProps {
  ...
  onChange: (v: HabitFormValues) => void;
}

const HabitFormFields = memo(({ values, onChange }: HabitFormFieldsProps) => (
  <div className="space-y-4">
    <div className="space-y-2">
      ...
      <Input
        ...
        placeholder="Ex: Treinar, Meditar, Ler"
      />
    </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {Object.entries(attrConfig).map(([key, cfg]) => (
          <button
            key={key}
            type="button"
            onClick={() => onChange({ ...values, attribute: key })}
            className={`p-2 rounded-lg border text-center text-xs transition-all ${
              (values.attribute || "productivity") === key
    ...
    <div className="grid grid-cols-2 gap-3">
      <div className="space-y-2">
        ...
        <Select
          ...
          onValueChange={(v) => {
            ...
            if (v === "weekdays")
              updated.custom_days = ["seg", "ter", "qua", "qui", "sex"];
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          onChange={(e) =>
            onChange({ ...values, xp_reward: Number(e.target.value) })
          }
          className="bg-secondary border-border"
        />
      </div>
  ...
  ({
    ...
    return (
      <motion.div
        ...
        className={`rounded-xl border p-4 transition-all group relative overflow-hidden ${
          isDone
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <p
                className={`font-semibold text-sm transition-all ${isDone ? "line-through text-muted-foreground" : "text-foreground"}`}
              >
                {habit.name}
              </p>
              <span
                className={`text-xs px-1.5 py-0.5 rounded-md ${attr.bg} ${attr.color}`}
              >
            ...
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              <span className="flex items-center gap-1">
                <Flame
                  className={`h-3 w-3 ${habit.streak > 0 ? "text-orange-400" : ""}`}
                />
                <span
                  className={
                    habit.streak > 0 ? "text-orange-400 font-semibold" : ""
                  }
                >
              ...
              <span className="flex items-center gap-1">
                <Zap className="h-3 w-3" /> {habit.xp_reward} XP
              </span>
            ...
            <div className="flex gap-1 mt-3">
              {weekDays.map((day) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-[10px] ${day.isToday ? "text-primary font-bold" : "text-muted-foreground"}`}
                    >
                      {day.label}
                    </span>
                    <div
...
const defaultFormValues: HabitFormValues = {
  ...
  frequency: "daily",
  xp_reward: 5,
  custom_days: ["seg", "ter", "qua", "qui", "sex"],
};
...
export default function HabitsPage() {
  ...
  const [formValues, setFormValues] =
    useState<HabitFormValues>(defaultFormValues);
  const [xpPopup, setXpPopup] = useState<{ amount: number; id: string } | null>(
    null,
  );
  ...
  const fetchData = async () => {
    ...
    if (logsRes.data)
      setCompletedToday(new Set(logsRes.data.map((l) => l.habit_id)));
    if (weekLogsRes.data) {
  ...
  const createHabit = async () => {
    ...
    if (!canCreate("habits", habits.length)) {
      toast({
        ...
        description: "Faça upgrade para criar mais hábitos.",
      });
      return;
    }
    const days =
      formValues.frequency === "custom" ? formValues.custom_days : null;
    await supabase.from("habits").insert({
      ...
      xp_reward: formValues.xp_reward || 5,
      custom_days: days,
    } as any);
  ...
  const handleEditOpen = (habit: Habit) => {
  ...
  const totalXpToday = activeHabits
    .filter((h) => completedToday.has(h.id))
    .reduce((a, h) => a + h.xp_reward, 0);
      ...
      >
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <Repeat className="h-6 w-6 text-primary" /> Hábitos
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Construa consistência e evolua seus atributos
          </p>
        </div>
        ...
        <Dialog
          ...
          onOpenChange={(open) => {
            setDialogOpen(open);
            if (!open) setFormValues(defaultFormValues);
          }}
        >
          ...
          <DialogContent className="bg-card border-border">
            ...
            <HabitFormFields values={formValues} onChange={setFormValues} />
            <Button onClick={createHabit} className="w-full mt-2">
              Criar Hábito
            </Button>
          </DialogContent>
        </Dialog>
        ...
        <DialogContent className="bg-card border-border">
          ...
          {editingHabit && (
            <>
              <HabitFormFields
                values={editingHabit}
                onChange={(v) => setEditingHabit(v as Habit)}
              />
              <div className="flex items-center gap-2 mt-2">
                <Checkbox
                  checked={editingHabit.is_active}
                  onCheckedChange={(v) =>
                    setEditingHabit({ ...editingHabit, is_active: !!v })
                  }
                />
          ...
          {progress === 100 && total > 0 && (
            ...
            >
              ...
              <span className="text-sm text-primary font-semibold">
                Todos os hábitos concluídos! Dia perfeito! 🔥
              </span>
              <Sparkles className="h-4 w-4 text-primary" />
            </motion.div>
          )}
          ...
          {activeHabits.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-sm opacity-70">
                Crie seu primeiro hábito para começar a evoluir!
              </p>
            </div>
          )}
        </TabsContent>
