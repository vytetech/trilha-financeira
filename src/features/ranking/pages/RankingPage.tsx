...
import GlobalLeaderboard from "@/features/ranking/components/GlobalLeaderboard";
...
const ACHIEVEMENTS: AchievementDef[] = [
  {
    id: "first_step",
    name: "Primeiro Passo",
    desc: "Criou sua conta no TRILHA",
    icon: "🎯",
    category: "geral",
    check: () => true,
    xp: 10,
  ...
  },
  {
    id: "tasks_10",
    name: "Executor",
    desc: "Concluiu 10 tarefas",
    icon: "✅",
    category: "produtividade",
    check: (d) => d.tasksDone >= 10,
    xp: 20,
  },
  {
    id: "tasks_50",
    name: "Máquina de Produção",
    desc: "Concluiu 50 tarefas",
    icon: "⚡",
    category: "produtividade",
    check: (d) => d.tasksDone >= 50,
    xp: 75,
  },
  {
    id: "tasks_100",
    name: "Executor Implacável",
    desc: "Concluiu 100 tarefas",
    icon: "💎",
    category: "produtividade",
    check: (d) => d.tasksDone >= 100,
    xp: 150,
  },
  {
    id: "tasks_500",
    name: "Lenda da Produtividade",
    desc: "Concluiu 500 tarefas",
    icon: "🔱",
    category: "produtividade",
    check: (d) => d.tasksDone >= 500,
    xp: 500,
  },
  {
    id: "habits_7",
    name: "Semana Forte",
    desc: "7 check-ins de hábitos",
    icon: "🔥",
    category: "habitos",
    check: (d) => d.totalHabitLogs >= 7,
    xp: 15,
  },
  {
    id: "habits_30",
    name: "Mês Disciplinado",
    desc: "30 check-ins de hábitos",
    icon: "💪",
    category: "habitos",
    check: (d) => d.totalHabitLogs >= 30,
    xp: 50,
  },
  {
    id: "habits_100",
    name: "Centurião",
    desc: "100 check-ins de hábitos",
    icon: "🛡️",
    category: "habitos",
    check: (d) => d.totalHabitLogs >= 100,
    xp: 100,
  },
  {
    id: "streak_7",
    name: "7 Dias Consistentes",
    desc: "Streak de 7 dias em algum hábito",
    icon: "🔗",
    category: "habitos",
  ...
  },
  {
    id: "streak_60",
    name: "Inabalável",
    desc: "Streak de 60 dias ininterrupto",
    icon: "🏔️",
    category: "habitos",
  ...
  },
  {
    id: "habits_5_active",
    name: "Multi-Hábitos",
    desc: "5 hábitos ativos simultaneamente",
    icon: "🎪",
    category: "habitos",
    check: (d) => d.habitsActive >= 5,
    xp: 30,
  },
  {
    id: "first_income",
    name: "Primeira Receita",
    desc: "Registrou sua primeira receita",
    icon: "💵",
    category: "financeiro",
    check: (d) => d.totalIncome > 0,
    xp: 10,
  },
  {
    id: "saver",
    name: "Poupador",
    desc: "Economizou mais de 20% da renda",
    icon: "🐷",
    category: "financeiro",
    check: (d) =>
      d.totalIncome > 0 &&
      (d.totalIncome - d.totalExpenses) / d.totalIncome >= 0.2,
    xp: 50,
  },
  {
    id: "investor",
    name: "Investidor Iniciante",
    desc: "Registrou primeiro investimento",
    icon: "📈",
    category: "financeiro",
    check: (d) => d.investmentsCount > 0,
    xp: 30,
  },
  {
    id: "investor_5",
    name: "Carteira Diversificada",
    desc: "5 investimentos na carteira",
    icon: "📊",
    category: "financeiro",
    check: (d) => d.investmentsCount >= 5,
    xp: 75,
  },
  {
    id: "transactions_50",
    ...
    icon: "📝",
    category: "financeiro",
    check: (d) => d.totalTransactions >= 50,
    xp: 40,
  },
  {
    id: "transactions_200",
    name: "Mestre das Finanças",
    desc: "200 transações registradas",
    icon: "🏦",
    category: "financeiro",
    check: (d) => d.totalTransactions >= 200,
    xp: 100,
  },
  {
    id: "goal_1",
    name: "Meta Concluída",
    desc: "Concluiu sua primeira meta",
    icon: "🏆",
    category: "metas",
  ...
  },
  {
    id: "goals_5",
    name: "Conquistador",
    desc: "Concluiu 5 metas",
    icon: "🎖️",
    category: "metas",
    check: (d) => d.goalsCompleted >= 5,
    xp: 100,
  },
  {
    id: "goals_10",
    ...
    icon: "🚀",
    category: "metas",
    check: (d) => d.goalsCompleted >= 10,
    xp: 200,
  },
  {
    id: "dream_1",
    name: "Sonho Realizado",
    desc: "Realizou seu primeiro sonho",
    icon: "✨",
    category: "metas",
    check: (d) => d.dreamsCompleted >= 1,
    xp: 75,
  },
  {
    id: "dream_saver",
    name: "Guardião dos Sonhos",
    desc: "Guardou R$1.000+ para sonhos",
    icon: "💫",
    category: "metas",
    check: (d) => d.totalDreamsSaved >= 1000,
    xp: 50,
  },
  {
    id: "dream_saver_10k",
    name: "Grande Sonhador",
    desc: "Guardou R$10.000+ para sonhos",
    icon: "🌠",
    category: "metas",
    check: (d) => d.totalDreamsSaved >= 10000,
    xp: 150,
  },
];
...
export default function RankingPage() {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      const [
        ...
        supabase
          .from("habits")
          .select("streak, best_streak, is_active")
          .eq("user_id", user.id),
      ...
      const tasksDone =
        tasksRes.data?.filter((t) => t.status === "done").length || 0;
      const totalHabitLogs = habitLogsRes.data?.length || 0;
      const habitsData = habitsRes.data || [];
      const bestStreak = habitsData.reduce(
        (max, h) => Math.max(max, h.best_streak || 0),
        0,
      );
      const avgStreak =
        habitsData.length > 0
          ? Math.round(
              habitsData.reduce((a, h) => a + (h.streak || 0), 0) /
                habitsData.length,
            )
          : 0;
      const habitsActive = habitsData.filter((h) => h.is_active).length;
      const goalsData = goalsRes.data || [];
      const goalsCompleted = goalsData.filter(
      ...
      const daysActive = profileRes.data
        ? Math.ceil(
      ...
      setStats({
        ...
        dreamsCompleted: dreamsData.filter((d) => d.status === "completed")
          .length,
        totalDreamsSaved: dreamsData.reduce(
          (a, d) => a + Number(d.current_amount),
          0,
        ),
        level: profileRes.data?.level || 1,
        xp: profileRes.data?.xp || 0,
        // xp_total — nunca reseta, usado para ordenar o ranking global
  ...
  const levelInfo =
    LEVEL_TITLES.find((l) => (profile?.level || 1) >= l.min) ||
    LEVEL_TITLES[LEVEL_TITLES.length - 1];

  // xp = XP dentro do nível atual | nextLevelXP = threshold do nível
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      >
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Acompanhe sua evolução e compete com outros jogadores
        </p>
      </motion.div>
      ...
      <Tabs value={rankingTab} onValueChange={setRankingTab}>
        ...
        <TabsContent value="global" className="mt-0">
          {/* GlobalLeaderboard deve ordenar por xp_total DESC */}
          <GlobalLeaderboard />
        </TabsContent>
        ...
        <TabsContent value="pessoal" className="mt-0 space-y-6">
          ...
          {profile && (
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center gap-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {/* Mostra XP total acumulado no perfil */}
                ...
                {/* Barra de progresso usa xp do nível atual */}
                <div className="flex-1 space-y-2">
                  ...
                  <div className="flex justify-between text-xs">
                    {/* xp = dentro do nível atual, nextLevelXP = threshold */}
                    <span className="text-muted-foreground">
                      {xp} / {nextLevelXP} XP neste nível
                    </span>
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  ...
                  {/* XP total em destaque — é o que importa no ranking */}
                  <div className="p-3 rounded-lg bg-card border border-border">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {[
              ...
              },
              {
                ...
                label: "Metas",
                value: `${stats.goalsCompleted}/${stats.goalsActive + stats.goalsCompleted}`,
              },
              {
          ...
          <Tabs defaultValue="todas">
            ...
            {["todas", "produtividade", "habitos", "financeiro", "metas"].map(
              (tab) => (
                        ...
                        {earned.length > 0 && (
                          <div className="space-y-2">
                            <p className="text-xs text-primary font-medium flex items-center gap-1">
                              <CheckCircle2 className="h-3 w-3" /> Desbloqueadas
                              ({earned.length})
                            </p>
                              ...
                              {earned.map((ach, i) => (
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    ...
                                    <Badge className="bg-primary/20 text-primary border-none text-xs shrink-0">
                                      +{ach.xp} XP
                                    </Badge>
                                  </div>
                                </motion.div>
                        ...
                        {locked.length > 0 && (
                          <div className="space-y-2">
                            <p className="text-xs text-muted-foreground font-medium flex items-center gap-1">
                              <Shield className="h-3 w-3" /> Bloqueadas (
                              {locked.length})
                            </p>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                              {locked.map((ach, i) => (
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <h3 className="font-semibold text-foreground mb-4 flex items-center gap-2">
              <Activity className="h-5 w-5 text-primary" /> Progresso por
              Categoria
            </h3>
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2 text-sm">
                      <span className="text-primary">
                        {CATEGORY_META[cat.category].icon}
                      </span>
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <p className="text-xs text-muted-foreground mt-3 pt-3 border-t border-border">
              💡 O <span className="text-foreground font-medium">XP Total</span>{" "}
              nunca reseta — é ele que determina sua posição no ranking global.
              O <span className="text-foreground font-medium">XP do nível</span>{" "}
              mostra seu progresso até o próximo nível.
            </p>
          </div>
        </TabsContent>
      </Tabs>
